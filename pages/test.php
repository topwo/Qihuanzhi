...
$revival = function(&$m0,&$m1,&$m2,&$m3,&$m4,&$m5,&$m_battle_cards)
{
    $tmp2 = array();
    ...
    for ($g=0; $g <=5 ; $g++)
    {
        ...
        if (!isset($$t))
        {
            if (in_array($g, $m_battle_cards))
            {
                $tmp2[$g] = $g;
            }
        }
    }
    return array_keys($tmp2);
};
                ...
                if(isset($$e))
                {
                    ...
                    if ($k % 3 == 1)
                    {
                        switch ($$m['skillActive']['type'])
                        {
                            ...
                            case '3':
                                if ( count($revival($m0,$m1,$m2,$m3,$m4,$m5,$m_battle_cards)) > 0 )
                                {
                                    $iu = $revival($m0,$m1,$m2,$m3,$m4,$m5,$m_battle_cards)[0];
                                    $t = 'm'.$iu;

                                    // 创建卡牌数据
                                    $m_current_count -= 1;
                                    $current_card_id = ex($u_info['team1.cards'])[$iu];
                                    $cards = get_data_array_byid('cards','cards', $current_card_id-1);
                                    $cards_coefficient = get_data_array_byid('cards.param','cards.coefficient', $current_card_id-1);
                                    $cards_skillNormal = get_data_array_byid('cards.skill','cards.skill',$cards['skillNormal']-1);
                                    $cards_skillActive = get_data_array_byid('cards.skill','cards.skill',$cards['skillActive']-1);

                                    array_push($m_battle_cards, $iu);
                                    ...
                                    echo 'm'.$$m['id'].'发动',$$m['skillActive']['name'],'，复活了，并恢复m',$iu,' ',round($$m['att'] * $$m['skillActive']['coe.cure']),'生命，';
                                    echo 'm'.$iu.'生命',$$t['hp'] = $$t['hp'] + round($$m['att'] * $$m['skillActive']['coe.cure']);
